...
import {
  ...
  crearReporte,
  deleteReportes,
} from "../services/reportesService.mjs";
...
export default function Reportes() {
  ...
  const toggleSelect = (idRepor) => {
    setSelectedIds((prev) =>
      prev.includes(idRepor)
        ? prev.filter((x) => x !== idRepor)
        : [...prev, idRepor],
    );
  };
  ...
  const crearReporteHandler = async ({
    tipo,
    id_producto,
    fecha_desde,
    fecha_hasta,
    ...
    /* ORIGINAL SERVICE CALL (Commented for Demo)
    try {
      const payload = { tipo, id_producto: Number(id_producto), fecha_desde, fecha_hasta };
      const resp = await crearReporte(payload);
      setMessageModal({ isOpen: true, title: "✔ Reporte creado", text: `Reporte ${resp.id} generado correctamente.`, type: "success" });
      await loadReportes();
      return { ok: true };
    } catch (err) {
      setMessageModal({ isOpen: true, title: "❌ Error", text: err.message || "Error al generar el reporte.", type: "error" });
      return { ok: false };
    }
    */

    // --- MOCK CREATION FOR DEMO ---
    const mockId = `REP-${Math.floor(Math.random() * 1000)}`;
    setMessageModal({
      isOpen: true,
      title: "✔ Reporte creado",
      text: `Reporte ${mockId} generado correctamente (Demo Mode).`,
      type: "success",
    });

    setReportes((prev) => [
      {
        id_reporte: Math.random(),
        id: mockId,
        tipo,
  ...
  const performDeleteSelected = async () => {
    ...
    try {
      ...
      setReportes((prev) =>
        prev.filter((r) => !selectedIds.includes(r.id_reporte)),
      );
    ...
    toPrint.forEach((rep) => {
      try {
        ...
        const rows = [
          ...
          [
            "Desde",
            (src.fecha_desde || src.desde || "-").toString().slice(0, 10),
          ],
          [
            "Hasta",
            (src.fecha_hasta || src.hasta || "-").toString().slice(0, 10),
          ],
          [
            "Cantidad (unidades)",
            String(src.cantidadUnidad ?? src.cantidad ?? "-"),
          ],
