...
export default function StockPesaje() {
  ...
  const loadMateriales = useCallback(async () => {
    ...
    */
    ...
    const mockMaterials = [
      { id_producto: 1, nombre: "Cartón Corrugado", unidad_stock: "kg" },
      { id_producto: 2, nombre: "Plástico PET", unidad_stock: "kg" },
      { id_producto: 5, nombre: "Plástico Film", unidad_stock: "kg" },
    ];
    ...
    */
    ...
    setTimeout(() => {
      setMessageModal({
        isOpen: true,
        title: "✅ Pesaje registrado",
        text: "El stock de los materiales fue actualizado correctamente (Demo Mode).",
        type: "success",
      });
    ...
    <PageContainer title="Registro de Pesaje">
      <div className="space-y-5 md:space-y-6 pb-28 md:pb-12">
        ...
        <div className="space-y-4">
          {/* Versión desktop */}
          <div className="hidden md:block bg-white rounded-2xl border border-[#e3e9e5] shadow-sm p-3 md:p-4">
            <DataTable
              columns={columns}
              data={items}
              zebra={false}
