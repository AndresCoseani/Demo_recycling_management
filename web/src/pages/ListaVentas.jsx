...
export default function Ventas() {
  ...
  */
  ...
  const loadVentas = useCallback(async () => {
    ...
    const mockVentas = [
      ...
      },
      {
        id_venta: 43,
        tipo: "Material",
        fecha: "2026-02-22T09:15:00Z",
        total: 85000,
        estado: "ACTIVO",
        observaciones: "Venta de excedente de cartón",
        productos: [
          {
            producto: "Cartón Prensado",
            cantidad: 500,
            medida: "kg",
            precio: 170,
            subtotal: 85000,
  ...
  const visibleRowsMobile = ventasFiltradas.slice(
    startMobile,
    startMobile + PAGE_SIZE_MOBILE,
  );
  ...
  const handleAnular = async () => {
    ...
    */
    ...
    // --- MOCK ANULAR FOR DEMO ---
    if (!ventaIdToAnular) return;
    setAnularConfirmOpen(false);
    setMessageModal({
      isOpen: true,
      title: "✅ Venta Anulada",
      text: `La venta N° ${ventaIdToAnular} ha sido anulada correctamente (Demo Mode).`,
      type: "success",
    });
    setVentaIdToAnular(null);
  };
  ...
  /* REMOVED DUPLICATE loadVentas IMPLEMENTATION FOR DEMO */
          ...
          <>
            ...
            <div className="md:hidden space-y-3">
              ...
              {visibleRowsMobile.map((row) => {
                ...
                const formattedTotal = Number(row.total || 0).toLocaleString(
                  "es-AR",
                  {
                    style: "currency",
                    currency: "ARS",
              ...
              {remitos.map((remito) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white border rounded-lg p-3 shadow-sm ${isSelected ? "border-emerald-500 ring-1 ring-emerald-500" : "border-slate-200"}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={() => {
                          const isActive =
                            remitoSel?.id_remito === remito.id_remito;
                          if (isActive) {
                            setRemitoSel(null);
                          } else {
                            setRemitoSel(remito);
