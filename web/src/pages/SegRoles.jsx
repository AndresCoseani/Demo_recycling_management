...
export default function SegRoles() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      /* ORIGINAL SUPABASE CALL (Commented for Demo)
      ...
      */
      ...
      const mockUsuarios = [
        { id_usuario: 1, nombre: "Administrador Sistema", id_rol: 1 },
        { id_usuario: 2, nombre: "Paula Gestión", id_rol: 2 },
        { id_usuario: 3, nombre: "Operario Planta", id_rol: 3 },
      ];

      setRoles(mockRoles);
      setUsuarios(mockUsuarios);

      const inicial = mockUsuarios.map((u) => ({
        id: u.id_usuario,
        rol: mockRoles.find((r) => r.id_rol === u.id_rol)?.nombre || "",
        usuario: u.nombre,
      }));
      setRows(inicial);
    };
  ...
  const onAssign = async () => {
    ...
    */
    ...
    setMessageModal({
      isOpen: true,
      title: " Rol asignado",
      text: `El usuario "${usr.nombre}" ahora tiene el rol "${rol.nombre}" (Demo Mode).`,
      type: "success",
    });
  };
  ...
  const onRemove = useCallback(async (row) => {
    ...
    setMessageModal({
      isOpen: true,
      title: " Rol removido",
      text: `Se removió el rol del usuario correctamente (Demo Mode).`,
      type: "success",
    });
  }, []);
      ...
      <div>
        ...
        <div className="md:hidden space-y-3">
          ...
          {rows.map((row) => (
            <div
              key={row.id}
              className="bg-white border p-4 rounded-lg shadow-sm border-slate-200"
            >
