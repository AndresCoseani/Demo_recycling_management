...
export default function Proveedores() {
      ...
      const mockProveedores = [
        {
          id_proveedor: 1,
          cuit: "30-11111111-9",
          nombre: "Reciclados Sur",
          contacto: "ventas@recicladossur.com",
          direccion: "Av. Industrial 450",
        },
        {
          id_proveedor: 2,
          cuit: "30-22222222-9",
          nombre: "EcoPackaging S.A.",
          contacto: "info@ecopack.com.ar",
          direccion: "Calle 14 N° 821",
        },
        {
          id_proveedor: 3,
          cuit: "30-33333333-9",
          nombre: "Packaging Supplies",
          contacto: "compras@gmail.com",
          direccion: "Ruta 20 km 5.5",
        },
        {
          id_proveedor: 4,
          cuit: "33-44444444-9",
          nombre: "Pritty S.A.",
          contacto: "logistica@pritty.com.ar",
          direccion: "Bv. Los Talleres 1200",
        },
      ];

      const mapped = mockProveedores.map(mapProveedorFromApi);
      setRows(mapped);
      setLoading(false);
    }
  ...
  async function handleSubmit(e) {
    ...
    try {
      setSaving(true);
      /* ORIGINAL API LOGIC (Commented for Demo)
      const payload = {
      ...
      */
      ...
      const updatedProveedor = {
        id: editing?.id ?? Math.floor(Math.random() * 1000),
        cuit: String(form.cuit || "").trim(),
        nombre: String(form.nombre || "").trim(),
        contacto: String(form.contacto || "").trim(),
        direccion: String(form.direccion || "").trim(),
      ...
      openMessage({
        ...
        text: isEdit
          ? `El proveedor "${updatedProveedor.nombre}" fue actualizado correctamente (Demo Mode).`
          : `El proveedor "${updatedProveedor.nombre}" fue registrado correctamente (Demo Mode).`,
      });
    } catch (err) {
      openMessage({
        type: "error",
        title: "❌ Error",
        text: "Error al simular operacion.",
      });
    } finally {
      setSaving(false);
    }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      /* ORIGINAL DELETE LOGIC (Commented for Demo)
      ...
      */
      ...
      openMessage({
        ...
        text: `El proveedor "${deleteTarget.nombre}" fue eliminado correctamente (Demo Mode).`,
      });
    } catch (err) {
      openMessage({
        type: "error",
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex flex-col">
                  <h3 className="font-bold text-[#154734] text-base">
                    {row.nombre}
                  </h3>
